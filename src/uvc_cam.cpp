  ...
  width(_width), height(_height), fps(_fps), rgb_frame(NULL)
{
  ...
  try
  {
	...
	// why i can't set the focus here?
	// what if i set the focus value in the calibration file?
...
// new constructor to control the camera
Cam::Cam(const char *_device, mode_t _mode, int _width, int _height, int _fps, int _autoPriority, int _autoExposure, int _brightness, int _contrast, int _whiteBalanceTempAuto, int _whiteBalanceTemp, int _powerFreq, int _gain, int _sharpness, int _blacklightCompen, int _absolute, int _led1mode, int _led1Freq, int _saturation, int _focus)
: mode(_mode), device(_device),
  motion_threshold_luminance(100), motion_threshold_count(-1),
  width(_width), height(_height), fps(_fps), rgb_frame(NULL),
  autoPriority(_autoPriority), autoExposure(_autoExposure), 
  brightness(_brightness), contrast(_contrast), 
  whiteBalanceTempAuto(_whiteBalanceTempAuto), 
  whiteBalanceTemp(_whiteBalanceTemp), powerFreq(_powerFreq), 
  gain(_gain), sharpness(_sharpness), blacklightCompen(_blacklightCompen), 
  absolute(_absolute), led1mode(_led1mode), led1Freq(_led1Freq), 
  saturation(_saturation), focus(_focus)
{
  printf("opening %s\n", _device);
  if ((fd = open(_device, O_RDWR)) == -1)
    throw std::runtime_error("couldn't open " + device);
  ...
  try
  {
    ...
    //set_control(V4L2_CID_EXPOSURE_AUTO_NEW, 2);
    set_control(10094851, autoPriority); // Exposure, Auto Priority
    set_control(10094849, autoExposure); // Exposure, Auto
    //set_control(168062321, 0); //Disable video processing
    //set_control(0x9a9010, 100);
    //set_control(V4L2_CID_EXPOSURE_ABSOLUTE_NEW, 300);
    //set_control(V4L2_CID_BRIGHTNESS, 140);
    ...
    set_control(9963788, whiteBalanceTempAuto); // White Balance Temperature, Auto
	
	// commented out by wpf
    //set_control(9963802, whiteBalanceTemp); // White Balance Temperature
    set_control(9963800, powerFreq);  // power line frequency to 60 hz
    set_control(9963795, gain); // Gain
    set_control(9963803, sharpness); // Sharpness
    set_control(9963804, blacklightCompen); //Backlight Compensation
    
	//set_control(10094850, 250); // Exposure (Absolute)
	// why i can't set the focus here?
	// what if i set the focus value in the calibration file?
    //set_control(168062212, absolute); //Focus (absolute)
    //set_control(168062213, led1mode); //LED1 Mode
    //set_control(168062214, led1Freq); //LED1 Frequency
