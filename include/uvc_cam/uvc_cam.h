...
public:
  ...
  Cam(const char *device, mode_t _mode = MODE_RGB,
	...
	// a new constructor designed to receive the new param, which can control the param of camera, such as brightness and contrast
  Cam(const char *device, mode_t _mode,
      int _width, int _height, int _fps, int _autoPriority, int _autoExposure, int _brightness, int _contrast, int _whiteBalanceTempAuto, int _whiteBalanceTemp, int _powerFreq, int _gain, int _sharpness, int _blacklightCompen, int _absolute, int _led1mode, int _led1Freq, int _saturation, int _focus);
  ~Cam();
...
private:
  ...
  unsigned width, height, fps;
	...
	// additional param added by wpf
	int autoPriority, autoExposure, brightness, contrast, whiteBalanceTempAuto, whiteBalanceTemp, powerFreq, gain, sharpness, blacklightCompen, absolute, led1mode, led1Freq, saturation, focus; 
